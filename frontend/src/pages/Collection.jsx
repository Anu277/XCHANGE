        ...
        {isLoading ? (
          ...
          <div className="container">
            {products.map((item, index) => {
              return(
              <ProductItem key={index} item={item} name={item.name||item.title} id={item._id || item.asin} price={item.price} image={(item.image && item.image[0]) || item.url_image } shipping_information={item.shipping_information}/>
            )
          }
            )}
          </div>
